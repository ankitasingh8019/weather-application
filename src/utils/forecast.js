...
const forecast=(latitude,longitude,callback)=>{
    const url='https://api.darksky.net/forecast/cafa2fe8e78db2dd33e1e0172adf790e/'+latitude+','+longitude+'?units=si'
    request({url,json:true},(error,{body})=>{
        if(error){
            callback('Unable to connect to weather services due to no network connection.',undefined)
        }
        else if(body.error){
